...
export default function SaleSection({data}) {
    return (
        <div className="w-full">
            <h3 className="text-center text-3xl font-bold py-5 border-t-2 border-b-2 border-black my-10">BEST OF PROPERTIES FOR SALE</h3>
                ...
                {data.map((data) => {
                    return (
                        <div key={data.title} className="lg:p-1 lg:w-1/3 lg:px-1 lg:py-0 py-2">
                            ...
                            <p className="text-2xl font-bold text-yellow-500">&#8377;{data.Price}</p>
                            <p className="my-2 text-lg font-semibold">{data.estate.address}</p>
                            <div className="mt-4">
                                <Link href={`/sale/${data.slug.current}`}>
